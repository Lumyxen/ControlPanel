...
export const PIN_AUTH_ITERATIONS = 310000;
export const PIN_WRAP_ITERATIONS = 310000;
...
export async function deriveVaultMaterial(password, kdf = {}) {
...
export async function encryptVaultData(vaultObject, vaultEncKey, revision, timestamps = {}) {
	const payload = await encryptJsonWithKey(vaultObject, vaultEncKey);
	return {
		version: 1,
		alg: 'AES-256-GCM',
		revision,
...
export async function decryptVaultData(blob, vaultEncKey) {
...
export async function createMasterProof(vaultAuthKey, challenge) {
	return computeHmacHex(vaultAuthKey, `vault:master:${challenge}`);
}

export async function createPinProof(pinAuthKey, challenge) {
	return computeHmacHex(pinAuthKey, `vault:pin:${challenge}`);
}

export function createPinAuthKdf() {
	return {
		type: 'pbkdf2',
		hash: 'sha256',
...
export async function derivePinAuthKey(pin, pinAuthKdf) {
...
export async function createPinLocalRecord({
	deviceId,
	pin,
	pepper,
	vaultEncKey,
...
export async function unwrapPinLocalRecord(record, pin, pepper) {
