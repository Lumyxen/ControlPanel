...
function isSafeStyledTag(tag) {
	return /^<(?:span|u)\s+style="[^"]*">$/i.test(tag) || /^<div style="text-align:[^"]*">$/i.test(tag);
}

function hasSafeGeneratedClass(tag) {
	return /class="md-/.test(tag);
}
...
function isSafeInlineHtmlTag(tag) {
	if (/^<\/[a-zA-Z][a-zA-Z0-9:-]*\s*>$/i.test(tag)) return true;
	if (/^<\/?(strong|em|code|a|img|ul|ol|li|br|hr|sup|sub|del|s|u|table|thead|tbody|tr|th|td|mark|abbr)\b/i.test(tag)) return true;
	if (/^<a href="[^"]*" target="_blank" rel="noopener noreferrer">/i.test(tag)) return true;
	if (/^<\/?(?:svg|path|circle|rect|line|ellipse|polyline|polygon)\b/i.test(tag)) return true;
	if (isSafeStyledTag(tag)) return true;
	if (hasSafeGeneratedClass(tag)) return true;
	return false;
}

function readGeneratedHtmlBlock(src) {
	const openMatch = src.match(/^ {0,3}<(div|details|summary|table|blockquote|pre|ul|ol|li|p)\b[^>]*>/i);
	...
	while ((match = tagPattern.exec(src)) !== null) {
		...
		if (isClosing) depth--;
		if (depth === 0) {
			return src.slice(0, tagPattern.lastIndex);
		}
	}
...
function tokenize(src) {
	...
	while (src) {
		const blankMatch = src.match(/^(?:[ \t]*\n)+/);
		if (blankMatch) {
			src = src.substring(blankMatch[0].length);
			continue;
		}
		...
		const htmlBlock = readGeneratedHtmlBlock(src);
		if (htmlBlock) {
			const block = htmlBlock.trim();
			// Only allow explicitly known, injected HTML blocks from preprocessors
			const isSafe = hasSafeGeneratedClass(block) ||
			...
			src = src.substring(htmlBlock.length);
			continue;
		}
