...
import { isUnlocked, isFirstRun, syncFromOtherTabs, setupPassword, login } from '../../services/auth.js';
import { mountStarfield } from './starfield.js';
export async function mountLoginPage(root = document.body) {
	if (isUnlocked()) {
	...
	const canvas        = root.querySelector('#starfield');

	const stopStarfield = mountStarfield(canvas);
...
// ── Form submission ───────────────────────────────────────────────────────────
	...
	const handleSubmit = async (e) => {
		...
		if (firstRun && password !== confirmInput.value) {
		...
		setLoading(true);
		try {
			if (firstRun) await setupPassword(password);
	...
	return () => {
		...
		if (confirmInput) confirmInput.removeEventListener('input', clearError);
	};
}
